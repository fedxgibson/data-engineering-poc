...
- **Phase 5** (CI/CD + Azure) — mostly complete: deployed for real to Azure Container Apps via Terraform/Terragrunt ([infra/](infra/), Gruntwork-style `modules/` + `live/`), live at `https://ca-portintel-dev--96is9kz.nicemushroom-b8b4d37f.swedencentral.azurecontainerapps.io` — `/health`, `/sap/PortCallSet`, and `/query` all verified responding from Azure, not localhost. Real deployment problems hit and fixed are in [domain/08-phases.md](domain/08-phases.md#real-evidence-applied-to-a-real-azure-subscription). GitHub Actions still open.
